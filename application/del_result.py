...
def del_result(grade, login, prev_res):
    if request.method == 'GET':
        return render_template('del_result.html', logged=session.get('login', False), login=session['login'], grade=b64dec(grade),
                               enc_grade=grade, testee_login=login, back_url=url_for('grade', name=grade), prev_res=prev_res, title='Удаление результата')
